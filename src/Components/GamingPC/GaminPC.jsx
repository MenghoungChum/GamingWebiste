...
import { computerGaming } from '../ProductList/ProductList'
...
const GaminPC = () => {
  ...
  return (
    <section>
       <Banner imageBanner={ComputerBanner} title="Computer Gaming"/>
        <div className='max-w-[1440px] mx-auto px-10 py-10 flex flex-wrap gap-3 md:gap-5 lg:gap-7'>
            {
            computerGaming.map(item=>(
            <div
            data-aos="zoom-in"
            data-aos-delay={item.id*200}
             key={item.id} className={`lg:w-[23%] w-full md:w-[31%] lg:h-[440px] md:h-[430px] h-[400px] bg-zinc-800 rounded-2xl cursor-pointer group border dark:bg-transparent overflow-hidden transition-all duration-300 ease-in-out ${item.shadow} ${item.borderColor}`}>
                <div className='w-full pt-5 relative h-[60%] cursor-pointer overflow-hidden'>
                    ...
                    <div>
                        <button 
                            ...
                            className={`text-white z-20 px-2 py-1 rounded-lg absolute left-[50%] transform translate-x-[-50%] bottom-4  flex items-center cursor-pointer lg:gap-2 text-sm lg:text-base opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out ${item.bgBtn} ${item.btnShadow}`}><span><LuShoppingCart /></span><span>Add to Card</span>
                        </button>
                    </div>
                    {/* Overlay */}
                    <div className='w-full h-full bg-linear-to-t opacity-0 group-hover:opacity-100 transition-all duration-300 ease-in-out from-black to-transparent absolute inset-0 pointer-events-none'></div>
                </div>
